...
describe("Account Data!", async () => {
  ...
  const programAddress = address(
    "3sUYZvPgx54AYYDSCrfgzTZ2W9kwH3pBg3H3sKsovgXb",
  );
  ...
  const transactionMessage = pipe(
    ...
    (tx) => appendTransactionMessageInstructions(programTx, tx),
     (tx) => addSignersToTransactionMessage([dataAccount], tx),
  );
  ...
  test("Close the address info account", async () => {
    const programTx: Instruction[] = [
    {
      programAddress,
      accounts: [
